...
class FederatedServer:
    ...
    def setup_routes(self):
        ...
        async def get_prototypes():
            ...
            return {
                ...
                "count": len(self.prototype_labels)
            }
        ...
        async def aggregate_models():
            """手动触发模型聚合"""
            return await self.perform_federated_averaging()

    # ==================== 核心业务逻辑 ====================
    async def handle_client_register(self, req: ClientRegister):
        """处理客户端报到"""
    ...
    async def handle_data_collection(self, submission: DataSubmission):
        """处理数据征收请求"""
...
def start_server(host="0.0.0.0", port=8000, n_prototypes=5):
    """启动联邦学习服务器"""
    server = FederatedServer(n_prototypes=n_prototypes)
    
    # 启动后台监控任务
    async def run_monitor():
        await server.status_monitor()
    ...
    ║ 地址: http://{host}:{port}           ║
    ║ 原型数: {n_prototypes}               ║
    ║ 监控间隔: {server.status_check_interval}秒  ║
    ╚══════════════════════════════════════╝
    """)
